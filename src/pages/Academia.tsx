...
(Continue para as 5 sugestões)`
          ...
          clientData: null // Fix for ChatRequestSchema validation (400 error)
        }),
      });
    ...
    <div className="p-4 md:p-8 h-full flex flex-col overflow-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        ...
        <AnimatedContainer animation="fade-up" delay={0.3}>
          <Card className="h-full border-chart-2/20 bg-chart-2/5">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg flex items-center gap-2">
      ...
      {isGenerating && (
        <AnimatedContainer animation="scale-in">
          <Card className="mb-6 border-primary/30 bg-gradient-to-r from-primary/5 via-chart-5/5 to-primary/5 animate-glow overflow-hidden relative">
                  ...
                  <div className="flex items-center gap-1 order-1 md:order-2">
                    ...
                    <div className="flex items-center gap-1">
                      {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                        // Simple page number logic for now, could be improved with ellipsis for many pages
                        ...
                        return (
                          <Button
                            ...
                            onClick={() => {
                              // usePagination doesn't have a goToPage, but we can simulate it if we had it.
                              // Since it only has next/prev, we'll just show the buttons for now.
                              // Actually, let's check usePagination.ts if it has goToPage.
                            }}
                ...
                <ScrollArea className="flex-1 pr-4">
                  <div className="space-y-3">
                    {paginatedData.map((suggestion) => (
                      <div
                        key={suggestion.id}
                        className="p-3 md:p-4 border border-border rounded-lg hover:border-primary/30 transition-colors cursor-pointer group relative"
                        onClick={() => setSelectedSuggestion(suggestion)}
                      >
                        <div className="space-y-2">
                          <div className="flex items-start justify-between gap-2">
                            <h3 className="font-semibold text-sm flex-1 group-hover:text-primary transition-colors">{suggestion.title}</h3>
                            <Button
                              variant="ghost"
                              size="icon"
                      ...
                      <div className="flex items-center gap-1 mx-2">
                        {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
                          <Button
                            ...
                            onClick={() => {
                              // We could add goToPage to usePagination if needed
                            }}
                            disabled={currentPage === pageNum}
                          >
                            {pageNum}
