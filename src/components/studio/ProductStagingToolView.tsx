...
interface ProductStagingToolViewProps {
    tool: StudioTool;
}

export function ProductStagingToolView({ tool }: ProductStagingToolViewProps) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const finalPrompt = customPrompt.trim()
                ? `${customPrompt}. Realistic environment product staging, lifestyle photography, natural home lighting, high-end interior design background, 8k resolution, cinematic lighting, photorealistic`
                : "Professional product staging in a realistic modern environment, lifestyle photography, natural lighting, high-end interior design background, 8k resolution, cinematic lighting, photorealistic";
    ...
    const handleDownload = async (image: StudioImage) => {
        try {
            ...
            a.href = url;
            a.download = `staging-${image.id.slice(0, 8)}.png`;
    ...
    const handleReuseGenerated = async (img: StudioImage) => {
        try {
            toast.loading('Preparando imagem...', { id: 'prep-img' });
            const res = await fetch(img.image_url);
            const blob = await res.blob();
            const file = new File([blob], `staging-${img.id}.png`, { type: blob.type || 'image/png' });
            setSelectedImage(file);
                                            ...
                                            >
                                                ...
                                                {offset === 0 && (
                                                    <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex items-end justify-center gap-2 opacity-0 hover:opacity-100 transition-opacity duration-300 h-[50%]">
                                                        <Button size="sm" onClick={(e) => { e.stopPropagation(); handleReuseGenerated(img); }} className="h-10 px-4 rounded-xl bg-primary hover:bg-primary/90 text-white shadow-lg text-sm flex-1 font-medium transform transition-transform hover:scale-105">
                                                            <Upload className="h-4 w-4 mr-2" /> Novo Cenário
                                                        </Button>
                            ...
                            <div className="flex flex-col items-center gap-6 animate-in fade-in duration-500">
                                <div className="relative flex items-center justify-center">
                                    <div className="bg-white p-6 rounded-2xl shadow-xl border border-slate-100 rotate-[-8deg] translate-x-12 z-10 w-[200px] aspect-square flex items-center justify-center overflow-hidden">
                                        <img src="https://images.unsplash.com/photo-1523275335684-37898b6baf30?auto=format&fit=crop&q=80&w=400" alt="Before" className="w-full h-full object-cover opacity-40" />
                                    </div>
                                    ...
                                    <div className="bg-white p-6 rounded-2xl shadow-2xl border border-slate-100 rotate-[8deg] -translate-x-12 z-10 w-[200px] aspect-square flex items-center justify-center overflow-hidden">
                                        <img src="https://images.unsplash.com/photo-1616486338812-3dadae4b4ace?auto=format&fit=crop&q=80&w=400" alt="After" className="w-full h-full object-cover" />
                                    </div>
                                </div>
                                <p className="text-secondary-foreground text-sm font-medium mt-4">
                                    Coloque os seus produtos em cenários realistas e profissionais
                                </p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-100 p-2 overflow-hidden flex flex-col gap-2 transition-all duration-300 hover:shadow-[0_8px_40px_rgb(0,0,0,0.16)]">
                            ...
                            {previewUrl ? (
                                <div className="flex items-center gap-4 w-full px-2">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-bold text-slate-700 truncate">Produto Selecionado</p>
                                ...
                                <div className="flex items-center gap-3 text-slate-500 group-hover:text-primary transition-colors">
                                    ...
                                    <span className="text-sm font-medium">Solte a foto ou <span className="text-primary font-semibold">selecione o produto</span></span>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="flex items-center gap-2 px-3 py-2 bg-slate-50/50 rounded-xl border border-slate-100/50">
                            <div className="flex-1 flex items-center min-w-0">
                                <input
                                    type="text"
                                    placeholder="Descreva o cenário (ex: sala de estar luxuosa)..."
                ...
                >
                    ...
                    <div className="absolute inset-x-0 bottom-0 p-8 md:p-12 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex items-end justify-center gap-4 z-[110] pointer-events-none">
                        <div className="pointer-events-auto flex items-center gap-4 animate-in slide-in-from-bottom-8 duration-500">
                            <Button size="lg" onClick={(e) => { e.stopPropagation(); setMaximizedImage(null); handleReuseGenerated(maximizedImage); }} className="h-12 px-6 rounded-xl bg-primary hover:bg-primary/90 text-white shadow-xl font-medium transform transition-transform hover:scale-105">
                                <Upload className="h-5 w-5 mr-2" /> Novo Cenário
                            </Button>
