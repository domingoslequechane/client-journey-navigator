...
interface RecolorToolViewProps {
    tool: StudioTool;
}
...
export function RecolorToolView({ tool }: RecolorToolViewProps) {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const result = await generateImage.mutateAsync({
                toolId: tool.id,
                prompt: `Change color to ${hexColor}`,
    ...
    const handleDownload = async (image: StudioImage) => {
        try {
            ...
            a.href = url;
            a.download = `recolor-${image.id.slice(0, 8)}.png`;
    ...
    const handleRecolorGenerated = async (img: StudioImage) => {
        try {
            toast.loading('Preparando imagem...', { id: 'prep-img' });
            const res = await fetch(img.image_url);
            const blob = await res.blob();
            const file = new File([blob], `recolor-${img.id}.png`, { type: blob.type || 'image/png' });
            setSelectedImage(file);
                    ...
                    <div className="max-w-xl w-full flex flex-col items-center gap-6">
                        ...
                        {showResult ? (
                            ...
                            <div className="flex flex-col items-center gap-6 animate-in fade-in duration-500">
                                ...
                                <img
                                    src="/inspiration/recolor_hero_graphic.png"
                                    alt="Recolor graphic"
            ...
            <AlertDialog open={!!deleteTarget} onOpenChange={(o) => !o && setDeleteTarget(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>Esta ação não pode ser desfeita e a imagem recolorida será apagada permanentemente.</AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
