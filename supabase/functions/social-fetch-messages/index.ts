...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (clientId) {
      const { data: client } = await serviceClient.from("clients").select("id, late_profile_id").eq("id", clientId).eq("organization_id", orgId).single();
      if (client?.late_profile_id) clientsToFetch = [{ id: client.id, late_profile_id: client.late_profile_id }];
    } else {
      const { data: clients } = await serviceClient.from("clients").select("id, late_profile_id").eq("organization_id", orgId).not("late_profile_id", "is", null);
    ...
    for (const client of clientsToFetch) {
      ...
      const { data: accounts } = await serviceClient.from("social_accounts").select("id, late_account_id, platform, client_id").eq("organization_id", orgId).eq("client_id", client.id);
      const accountMap = new Map((accounts || []).map((a: any) => [a.late_account_id, a]));

      for (const msg of lateMessages) {
        const account = accountMap.get(msg.account_id);
        if (!account) continue;

        const { error: insertError } = await serviceClient.from("social_messages").upsert({
          organization_id: orgId, social_account_id: account.id, client_id: client.id, platform: account.platform,
          message_type: msg.type === "comment" ? "comment" : "dm", post_id: msg.post_id || null,
          sender_name: msg.sender?.name || "Unknown", sender_username: msg.sender?.username || null,
          sender_avatar_url: msg.sender?.avatar_url || null, content: msg.content || msg.text || "",
          is_read: false, external_id: msg.id, received_at: msg.created_at || new Date().toISOString(),
